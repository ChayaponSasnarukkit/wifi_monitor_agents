...
def _is_ap_config_active(link_status: str, ssid: str, tx_power: int, request_body: ConfigureAccessPointData) -> bool:
    if int(link_status[test_ap_status.find("TX packets:") + 11]) > 0 and ssid == request_body.ssid and tx_power == request_body.tx_power:
        return True
    else:
        return False
    pass
...
async def is_ap_config_active(request_body: ConfigureAccessPointData) -> bool:
    ...
    return _is_ap_config_active(link_status, ssid, tx_power, request_body)   
